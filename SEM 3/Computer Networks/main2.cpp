...
void selectiveRepeatSlidingWindow::sender()
{
    event = frameArrival;
    printf("\n\nSENDER\n======");
    if (frameToSend == (err + (MAX_SEQ / 2)) % (MAX_SEQ + 1) && errorDetected == true && err >= 0)
    {
        event = timeout;
        frameToSend = err;
        errorDetected = 0;
    }
    else if (frameToSend == MAX_SEQ && frameToSend != frameExpected && errorDetected == true)
    {
...
int main()
{
    ...
    printf("Enter Data: ");
    scanf("%s", temp);
    selectiveRepeatSlidingWindow *obj = new selectiveRepeatSlidingWindow(pow(2, n) - 1, string(temp));
    obj->sender();
    delete obj;
    return 0;
}
