/*
QUESTION-13: Write a menu driven program to perform following operations on strings:
(g) Show address of each character in string.
(h) Concatenate two strings without using strcat function.
(i) Concatenate two strings using strcat function.
(j) Compare two strings.
(k) Calculate the length of the string (use pointers).
(l) Convert all lowercase characters to uppercase.
(m) Convert all uppercase characters to lowercase.
(n) Calculate the number of vowels.
...
int countVowels(char a[])
{
    ...
    char t[255];
    for (int i = 0; i < stringLength(a); i++)
        if (lowercase(a, t)[i] == 'a' || lowercase(a, t)[i] == 'e' || lowercase(a, t)[i] == 'i' || lowercase(a, t)[i] == 'o' || lowercase(a, t)[i] == 'u')
            count++;
    return count;
}
